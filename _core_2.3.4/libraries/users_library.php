 ...
 */
	...
	if(!function_exists("user_check_username")) {
		function user_check_username($username) {
			...
			$not_found = (db_query_row("SELECT id FROM " . USERS_TABLE . " WHERE username = '{$username}'") === false) && (db_query_row("SELECT id FROM " . USER_TEMP_TABLE . " WHERE username = '{$username_clean}'") === false);
