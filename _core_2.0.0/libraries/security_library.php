<?php if(!defined("CORE_FOLDER")) die("Hacking attempt logged from IP " . $_SERVER['REMOTE_ADDR']);
	/**
	 *	security_library.php
	 *	
	 *	Provides functions to help prevent CSRF (seasurf) attacks.
	 *	
	 *	Prerequisites: session_library, users_library
	 */
	load_libraries(array("session", "users"));
	...
	if(!function_exists("security_token")) {
		function security_token($object = "", $action = "", $id = 0) {
			return core_hash(APP_ID . session_key() . user_id() . $object . $action . $id, 12);
		}
	}
