<?php
	...
	/************************ Checks to make sure all settings are set ***********************/
	if(
		setting_isset('users', 'login_type', 'email')
		&& setting_isset("users", "min_password_length")
		&& setting_isset('users', 'last_active_frequency')
		&& setting_isset('users', 'remember_me_timeout')
		&& setting_isset('users', 'trust_timeout')
		&& setting_isset('users', 'login_failure_limit')
		&& setting_isset('users', 'password_reset_timeout')
		&& setting_isset('users', 'allow_registration')
		&& setting_isset("users", "activation_email_template")
		&& setting_isset("users", "reset_password_email_template")
	) { } else {
		die("user_library.php - user settings not correct in config.php. Please check that all settings are set.");
	}
	...
	if(!function_exists("user_login")) {
		function user_login($moniker, $password, $remember_me = false) {
		...
		function user_logged_in() {	
			if(session_isset('LOGIN_STATUS') && session('LOGIN_STATUS') == LOGGED_IN_VALUE) {
				...
				if(cookie_isset('remember_key')) {
					...
					$autologin_row = db_query_row("SELECT user_id, user_agent, ual.last_ip FROM " . USER_AUTOLOGIN_TABLE . " ual INNER JOIN " . USERS_TABLE . " u ON u.id = ual.user_id WHERE key_id = '{$remember_key_clean}'");
					if (is_array($autologin_row)) {
						if ($autologin_row['user_agent'] == $_SERVER['HTTP_USER_AGENT']	&& $autologin_row['last_ip'] == _user_ip()) {
							$user_row = user_get_by_id($autologin_row['user_id']);
							
							_user_login($autologin_row['user_id'], $user_row['display_name'], $user_row['is_admin']);
							_user_remove_remember_me_key();
							_user_add_remember_me_key($autologin_row['user_id']);
	...
	if(!function_exists("user_check_username")) {
		function user_check_username($username) {
			...
			return (db_query_row("SELECT id FROM " . USERS_TABLE . " WHERE username = '{$username}'") === false) && (db_query_row("SELECT id FROM " . USER_TEMP_TABLE . " WHERE username = '{$username_clean}'") === false);
		}
	}	
	...
	if(!function_exists("user_check_email")) {
		function user_check_email($email, $user_id = 0) {
			...
			if (db_query_row("SELECT id FROM " . USERS_TABLE . " WHERE email = '{$email_clean}' AND id != {$user_id}") === false && db_query_row("SELECT id FROM " . USER_TEMP_TABLE . " WHERE email like '{$email_clean}'") === false) {
				return $email_clean;
			} else {
	...
	if(!function_exists("user_activate")) {
		function user_activate($activation_key, $password) {
			_user_purge_user_temp();
