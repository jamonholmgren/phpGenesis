...
//		Tim Santeford - tsantef@gmail.com
	...
	if(!function_exists("form_textbox")) {
		function form_textbox($name = '', $label = '', $value = NULL, $options = array(), $enabled = FORM_ENABLED) {
			...
			return _form_field("<input type=\"text\" id=\"{$fname}\" name=\"{$fname}\" class=\"" . implode(" ", $classes) . " {$control_class}\" {$style_html} value=\"{$value}\" {$autocomplete} {$max_length_html} {$size_html} {$readonly_html} {$placeholder} {$custom_attr} />", $fname, $label, $options);
		}
	}
	...
	if(!function_exists("form_password")) {
		function form_password($name='', $label = '', $value = NULL, $options = array(), $enabled = FORM_ENABLED) {
			...
			return _form_field("<input type=\"password\" id=\"{$fname}\" name=\"{$fname}\" class=\"" . implode(" ", $classes) . " {$control_class}\" value=\"{$value}\" {$autocomplete} {$max_length_html} {$readonly_html} {$custom_attr} />", $fname, $label, $options);
		}
	}
	...
	if(!function_exists("form_textarea")) {
		function form_textarea($name = '', $label = '', $value = NULL, $options = array(), $enabled = FORM_ENABLED) {
			...
			return _form_field("<textarea id=\"{$fname}\" name=\"{$fname}\" class=\"" . implode(" ", $classes) . " {$control_class}\" {$rows_html} {$cols_html} {$max_length_html} {$readonly_html} {$custom_attr}>{$value}</textarea>", $fname, $label, $options);
		}
	}
	...
	if(!function_exists("form_field_name")) {
		function form_field_name($name) {
			...
			if(strlen($form_name) < 1) return $name;
		...
		function form_posted($name, $check_key_is_valid = true) {
			...
			if(input_post("{$name}_key") !== NULL) {
				...
				die("
					...
					<pre>" . form_key() . " Expected\n" . input_post("{$name}_key") . " Posted</pre>
				");
			}
			return false;
		}
	...
	/**
	 *	Validates form data but allows HTML (uses HTMLPurifier by default)
	 *	
	 */
	...
	if(!function_exists("form_validate_checkbox")) {
		function form_validate_checkbox($name, $true = TRUE, $false = FALSE) {
