 ...
 */
	...
	/**
	 * Connect to Database (used by db_init)
	 * 
	 * @return NULL
	 */
	...
	/**
	 * Runs a query to check if a table exists
	 * 
	 * @return bool
	 */
	...
	/**
	 * Alias for db_query_rows() but only returns first row
	 * 
	 * @return array
	 */
	...
	/**
	 * Friendly alias for db_query_row() but only returns requested field
	 * 
	 * @return string
	 */
	...
	/**
	 * Friendly alias for db_query_row()
	 * 
	 * @return array
	 */
	if(!function_exists("db_get_row")) {
		function db_get_row($table, $field, $value) {
			return db_query_row("SELECT * FROM {$table} WHERE {$field} = '{$value}' LIMIT 1");
		}
	} // end db_get_row

	/**
	 * Friendly alias for db_query_rows()
	 * 
	 * @return array
	 */
	...
	/**
	 * Inserts row into table. Returns id or FALSE.
	 * 
	 * @return int
	 */
	...
	if(!function_exists("db_upsert")) {
		function db_upsert($table, $updated_row, $where = NULL, $id_field = "id") {
			db_init();
			...
			$q = _db_log_query("
				...
				ON DUPLICATE KEY 
				UPDATE 
					{$id_field} = LAST_INSERT_ID({$id_field}){$update_list}
				{$where}
			");
	...
	/**
	 * Delete row where $where. Returns number of affected rows.
	 * 
	 * @return int
	 */
	...
	/**
	 * Friendly alias for db_delete_where(). Returns number of affected rows.
	 * 
	 * @return int
	 */
	...
	/**
	 * Updates entire row after stripping slashes
	 * 
	 * @return int
	 */
	...
	/**
	 * Returns an array of fieldnames and their datatypes?
	 * 
	 * @return array
	 */
	...
	/**
	 * Reorders table by specified field.
	 * 
	 * @return bool
	 */
	...
	/**
	 * Prints out error based on application status (set in config)
	 * 
	 * @return NULL
	 */
	...
	/**
	 *	Private function that stores the recent queries.
	 *	
	 *	@return string
	 */
	...
	/**
	 * Counts number of queries and how long (in miliseconds) each one takes.
	 * Returns HTML of results
	 * 
	 * @return string
	 */
	if(!function_exists("db_performance_monitor")) {
	...
	/**
	 * Prints the entire database log for that page load.
	 * 
	 * @return NULL
	 */
