<? if(!defined("CORE_FOLDER")) die("Hacking attempt logged from IP " . $_SERVER['REMOTE_ADDR']);
	...
	if(!function_exists("social_login_process")) {
		function social_login_process() {
			if(input_get("state")) {
				if(input_get("state") == session("social_login_state") || input_get("state") == cookie("social_login_state")) {
					...
					if(dev()) notice_add("error", input_get("state") . " != " . session("social_login_state"));
				}
			}
		}
	}
	...
	if(!function_exists("social_login_login_button")) {
		function social_login_login_button($text = "Sign In", $url = NULL) {
			if(!session("social_login_state") || !cookie("social_login_state")) {
				session("social_login_state", md5(uniqid(rand(), TRUE))); // CSRF protection
				cookie("social_login_state", session("social_login_state"));
			}
			if($url === NULL) {
			...
			return '<a class="rpxnow" id="social-login" onclick="return false;" href="https://' . settings("social_login", "username") . '.rpxnow.com/openid/v2/signin?token_url=' . $url . '">' . $text . '</a>';
		}
	}
