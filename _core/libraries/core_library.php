...
//	nothing for now
	...
	/**
	 *	Private function that parses the given URL into an array of segments
	 *	
	 *	@return array
	 */
	...
	/**
	 *	Private function that parses the given URL into an array($key => $segment)
	 *	
	 *	This groups segments into pairs - used by something in the core
	 *	
	 *	@return array
	 */
	...
	/**
	 *	If page access is blocked in settings(), redirect to 403 error page
	 *	
	 *	@return null
	 */
	...
	/**
	 *	Private function that strips the URL of the current page down to just a URI
	 *	
	 *	@return string
	 */
	...
	function redirect($to = "", $code = 303) {
		if(library_is_loaded("session")) session_cleanup();
	 ...
	 *
	 * Can be modified to return a different url if referring URL is invalid by passing in another URL.
	 *
	 * Invalid URLs include external domains or the current page (on a refresh).
	 *
	 *
	 * @return string/url
	 */
	if(!function_exists("previous_url")) {
		function previous_url($default = NULL) {
			$referrer = $_SERVER['HTTP_REFERER'];
			$host = $_SERVER['HTTP_HOST'];
			$return = $referrer;
			
			// If referrer is external domain
			if(strpos($referrer, $host) === FALSE || $referrer == BASE_URL . $_SERVER['REQUEST_URI']) {
				// If url provided for invalid situations
				if($default !== NULL) {
					$return = $go_to_if_external;
				}
				// Else - leave alone
			}
