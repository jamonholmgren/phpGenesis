 ...
 *	Form Library
 *	
 *	Everything you'll ever need to build a form. Everything is highly automated.
 *	
 ...
 */
	...
	if(!defined("DEFAULT_FIELD_TEMPLATE")) {
		define("DEFAULT_FIELD_TEMPLATE", "
			<div id=\"field_%field_name%\" class=\"%field_classes%\">
				<label for=\"%field_name%\" id=\"%label_id%\">%field_label%%required_html%</label>
				%control%
				%error_html%
				%help_html%		
			</div>
	...
	if(!defined("DEFAULT_CHECKBOX_FIELD_TEMPLATE")) {
		define("DEFAULT_CHECKBOX_FIELD_TEMPLATE", "
			<div id=\"field_%field_name%\" class=\"checkbox-field %field_classes%\">			
				%control%
				<label for=\"%field_name%\" id=\"%label_id%\">%field_label%%required_html%</label>
				%error_html%
				%help_html%		
	...
	if(!function_exists("form_open")) {
		function form_open($name, $action='', $method=NULL, $options = array(), $enabled_default = FORM_ENABLED) {
			...
			$active_record_object = FALSE;
			if(is_object($name)) {
				$active_record_object = $name;
				$name = get_class($active_record_object);
			}
			
			globals('form', 'current_form', $name);
			globals('form', 'current_form_active_record_object', $active_record_object);
	 ...
	 *	@return boolean
	 */
	if(!function_exists("form_value_checkbox")) {
		function form_value_checkbox($name, $true = 1, $false = 0) {
	...
	if(!function_exists("form_textbox")) {
		function form_textbox($name = '', $label = '', $value = NULL, $options = array(), $enabled = FORM_ENABLED) {
			...
			$label = ($label === TRUE) ? _form_generate_label($name) : $label;
			$fname = form_field_name($name);
			...
			/********/
				...
				foreach($options as $k => $v) { // Empty breaks are intentional to prevent unsetting
					switch ($k) {	
						case "class":
							break;
						case "error":
	...
	if(!function_exists("form_radio")) {
		function form_radio($name='', $values = array(), $checked_value=NULL, $options = array(), $enabled = FORM_ENABLED) {
			if(!is_array($values)) {
				var_dump($values);
				die("Wrong argument type for form_radio. Need array.");
			}			
			...
			foreach($keys as $index => $value) {
				$value = $keys[$index];
				$label = $values[$value];
				if($options['key_same_as_label'] == TRUE) $value = $values[$value];
			...
			if(is_array($values)) {
				foreach($values as $val => $name) {
					...
					if(is_array($name)) {
						...
						if(isset($options['option_title'])) $title = $name[$options['option_title']];
					...
					$option_html .= "<option value=\"{$sel_val}\" {$selected} title=\"{$title}\">{$sel_name}</option>";
				}
			}
	...
	if(!function_exists("form_hidden")) {
		function form_hidden($name = '', $value = '', $attributes = array()) {
			...
			if(isset($attributes['control_class'])) {
				$control_class = $attributes['control_class'];
				unset($attributes['control_class']);
			}
			...
			return "<input type=\"hidden\" name=\"{$fname}\" id=\"{$fname}\" value=\"{$value}\" class=\"{$control_class}\" {$attributes_html} />";
		}
	}
	...
	/**
	 *	Private function that returns html for the passed field type and options
	 *	
	 *	@return string
	 */
	if(!function_exists("_form_field")) {
		function _form_field($control='', $field_name='', $field_label='', $options = array()) {
			...
			if(isset($options['label_id']) && $options['label_id'] != '') {
			...
			// Errors
			$active_record_object = globals('form', 'current_form_active_record_object');
			$error_html = "";
			if (!(isset($options['ignore_error']) && $options['ignore_error'] === true)) {
				...
				} elseif(is_object($active_record_object) && is_object($active_record_object->errors)) {
					$field_finder = ($options['db_field_name']) ? $options['db_field_name'] : str_replace((globals("form", "current_form") . "_"), "", $field_name);
					if($active_record_object->errors->is_invalid($field_finder)) {
						$error_html = "<p class=\"error\">" . $active_record_object->errors->on($field_finder) . "</p>";
						if(strpos($options['class'], "error") == 0) {
							$field_classes .= " error ";
						}
					}
			...
			$template = str_replace("%label_id%", $label_id, $template);
	...
	if(!function_exists("form_field_name")) {
		function form_field_name($name) {
			...
			if((settings("form", "ajax_uses_simplified_names") && is_ajax()) || strlen($form_name) < 1) return $name;
	...
	if(!function_exists("form_posted")) {
		function form_posted($name, $check_key_is_valid = true) {
			...
			if(input_post("{$name}_key") !== NULL) {
				...
				");
			} elseif(is_ajax() && input_post("ajax_form") == $name) {
				load_library("security");
				if(security_check()) {
					globals("form", "current_form", "");
					globals('form', '_valid', true);
					return TRUE;
				}
			}
			return false;
	...
	/**
	 * Generates a label bassed on the passed field name
	 * 
	 * @return string
	 */
	if(!function_exists("_form_generate_label")) {
		function _form_generate_label($name) {
			$label = str_replace(array("-", "_", "id"), array(" ", " ", ""), $name);
