...
//	Set above with @todo
   ...
   * @param string $str Path to file or directory
	...
	/**
	*	Do this when you've run out of options.
	*
	*	Does pretty much what you'd expect.
	*/
	
	if(!function_exists("angrify_me")) {
		function angrify_me($content) {
			$content = "NULL";
			$angrify = "It didn't work!";
			$return = $angrify . "Input = " . $content . " " . "Surprise?";
			return $return;
		}
	}
	
	/**
	*	Alias for angrify_me()
	*
	*/
	
	if(!function_exists("angrify")) {
		function angrify($content) {
			angrify_me($content);
		}
	}
